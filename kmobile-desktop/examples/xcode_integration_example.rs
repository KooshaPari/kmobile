...
use kmobile_desktop::{
    DeviceBridge, HardwareEmulator, XcodeIntegration, XcodeConfig, XcodeWorkflow,
    xcode_integration::{WorkflowStep, BuildConfiguration},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, sim) in simulators.iter().enumerate().take(5) {
        println!("   {}. {} ({}) - {:?}", i + 1, sim.name, sim.runtime, sim.state);
    }
    ...
    if let Some(simulator) = simulators.first() {
        ...
        match xcode.simulate_location(&simulator.udid, 37.7749, -122.4194).await {
        ...
        match xcode.take_device_screenshot(&simulator.udid, &screenshot_path).await {
    ...
    if let Some(simulator) = simulators.first() {
        ...
        use kmobile_desktop::xcode_integration::{PushNotification, NotificationPriority};
        ...
        match xcode.simulate_push_notification(&simulator.udid, notification).await {
