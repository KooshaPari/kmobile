...
pub use app::{KMobileDesktopApp, AgentAction};
pub use audio::{AudioProcessor, VoiceSettings, AudioConfig};
pub use computer_vision::{ScreenAnalyzer, UiElement, UiElementType, ScreenAnalysisResult};
pub use device_bridge::{DeviceBridge, DeviceType, ConnectionType, ScreenshotData};
pub use hardware_emulator::{HardwareEmulator, DeviceHardwareState, AudioRouting, NetworkConditions, HapticPattern};
pub use xcode_integration::{XcodeIntegration, XcodeConfig, SimulatorInfo, PhysicalDevice, BuildResult, TestResult, XcodeWorkflow, WorkflowResult};
...
pub mod agent_api {
    ...
    impl AgentController {
        ...
        pub async fn get_device_state(&self) -> Result<DeviceState> {
            let device_id = self.connected_device.as_ref()
        ...
        pub async fn simulate_location(&self, latitude: f64, longitude: f64) -> Result<()> {
            tracing::info!("📍 Agent simulating GPS location: {}, {}", latitude, longitude);
        ...
        pub async fn tap_element(&self, element_identifier: &str) -> Result<()> {
            ...
            for element in &analysis.ui_elements {
                if let Some(text) = &element.text {
                    if text.to_lowercase().contains(&element_identifier.to_lowercase()) {
                        return self.tap_coordinates(
        ...
        pub async fn start_autonomous_mode(&self, objective: &str) -> Result<()> {
            tracing::info!("🤖 Agent starting autonomous mode with objective: '{}'", objective);
